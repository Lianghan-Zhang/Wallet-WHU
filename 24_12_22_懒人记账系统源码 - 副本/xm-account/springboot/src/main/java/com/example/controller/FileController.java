...
public class FileController {
    ...
    public Result editorUpload(MultipartFile file) {
        String flag;
        synchronized (FileController.class) {
            flag = System.currentTimeMillis() + "";
            ThreadUtil.sleep(1L);
        ...
        String url = "http://" + ip + ":" + port + "/files/" + flag + "-" + fileName;  //  http://localhost:9090/files/1697438073596-avatar.png
        Dict res = Dict.create().set("errno", 0).set("url", CollUtil.newArrayList(Dict.create().set("url", url)));
